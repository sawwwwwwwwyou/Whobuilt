            ...
            <div className="layout-grid" style={{ gridTemplateColumns: '320px minmax(0, 1fr)' }}>
                <aside className="sidebar-nav">
                    <div className="nav-card" style={{ marginBottom: '24px' }}>
                        ...
                        <div className="nav-list">
                            <Link href="/" style={{ display: 'block', padding: '12px 16px', fontSize: '0.9rem', fontWeight: 700 }}>
                                &lt;- BACK TO DIRECTORY
                            </Link>
                        </div>
                    </div>
                ...
                <article className="product-card" style={{ gridTemplateColumns: '1fr', display: 'flex', flexDirection: 'column', cursor: 'default' }}>
                    <div className="content-col" style={{ flex: 1 }}>
                        {/* Language switcher */}
                        {hasMultiLang && (
                            <div style={{ display: 'flex', borderBottom: 'var(--b-width) solid var(--c-border)', background: 'var(--c-bg-canvas)' }}>
                                            ...
                                            {article.snippets.length > 1 && (
                                                <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                                                    {article.snippets.map((s, idx) => (
                                                        <button key={idx} onClick={() => setActiveSnippetIdx(idx)} style={{ padding: '8px 16px', background: activeSnippetIdx === idx ? 'rgba(255,255,255,0.1)' : 'transparent', color: activeSnippetIdx === idx ? 'var(--c-accent)' : '#fff', border: 'none', cursor: 'pointer', fontFamily: 'var(--f-mono)', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase' }}>{s.lang}</button>
                                                    ))}
                                                </div>
                                            )}
                    ...
                    <div className="vote-col" style={{ flexDirection: 'row', borderLeft: 'none', borderTop: 'var(--b-width) solid var(--c-border)' }}>
                        <button className="vote-btn" onClick={() => setHasVoted(true)} style={{ flexDirection: 'row', gap: '12px', padding: '16px', flex: 1, color: hasVoted ? 'var(--c-accent)' : 'inherit' }}>
                        ...
                        <button
                            ...
                            onClick={() => setModal({ title: 'COMMENTS', message: `Comments for "${article.title}" are currently read-only. We are migrating to a new discussion platform. Stay tuned!` })}
